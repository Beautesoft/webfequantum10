...
export class ManualListQuotationClass extends React.Component {
  ...
  getQuotations = () => {
    ...
    let { quotationList, pageMeta, formField, page, limit, statusStr } =
      this.state;
    let { fromDate, toDate, custName, manualinv_number, projectName, status } =
      formField;
    ...
    this.props
      .getCommonApi(
        `manualinvoicelist/?searchfrom=${dateFormat(
          From,
          "yyyy-mm-dd"
        )}&searchto=${dateFormat(
          To,
          "yyyy-mm-dd"
        )}&searchtitle=${projectName}&searchstatus=${statusStr}&searchname=${custName}&searchnumber=${manualinv_number}&page=${page}&limit=${limit}`
      )
      .then(async res => {
    ...
    return (
      <div className="project-section col-md-12 align-items-center">
        <div className="col-md-12 d-flex">
          <p className="label-head mb-4">{t("Manual Invoice")}</p>
        </div>
        ...
        <div className="row m-0 filter">
          <div className="col-md-5 col-12">
            ...
            <div className="d-flex mb-2">
              <label className="text-left w-100 text-black common-label-text mr-2">
                {t("Manual Invoice Number")}
              </label>
              <div className="input-group">
                <NormalInput
                  value={manualinv_number}
                  name="manualinv_number"
                  ...
                  quotationList.map((item, index) => {
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <div className="d-flex align-items-center justify-content-center">
                            <Link to={`quantum/manualinvoice/${id}/edit`}>
                              {manualinv_number}
                            </Link>
                          </div>
                        </td>
